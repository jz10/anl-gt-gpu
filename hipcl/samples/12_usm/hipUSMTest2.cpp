...
int main() {
    ...
    ERR_CHECK;
    std::cout << "dev id = " << devId << std::endl;
    ...
    for (i = 0; i < NUM; i++) {
      Matrix1[i] = gpuMatrix1[i] = rnd();
      Matrix2[i] = gpuMatrix2[i] = rnd();
    }

    err = hipMemAdvise(gpuMatrix1, NUM * sizeof(float), hipMemoryAdvise::hipMemAdviseSetReadMostly, devId);
    ERR_CHECK;
    err = hipMemAdvise(gpuMatrix2, NUM * sizeof(float), hipMemoryAdvise::hipMemAdviseSetReadMostly, devId);
    ERR_CHECK;
